...
export class Soundscape {
  ...
  status() {
    if (this.error) return '오디오를 시작할 수 없어요 · M으로 재시도';
    if (this.muted) return '음소거 · M으로 켜기';
    if (!this.enabled) return '비행을 시작하면 사운드가 켜집니다 · M으로 미리 듣기';
    if (this.ctx?.state === 'suspended') return '오디오 대기 중 · M으로 재시도';
    if (this.paused && !this.keepAmbience) return '게임과 함께 일시정지됨';
    if (!this.ambienceVolume) return '환경음 볼륨이 0입니다';
    const audible = [...(this.ambient?.voices || [])].filter(v => v.asset === this.ambient.layers.get(v.role)?.asset).length;
    if (audible) return `환경음 재생 중 · ${audible}개 레이어 활성`;
    if (this.ambient?.failed.size) return '환경음을 사용할 수 없음 · M으로 재시도';
    return '환경음 로딩 중…';
  }
