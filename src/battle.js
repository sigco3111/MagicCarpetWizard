...
export class Battle {
  ...
  kill(e, run) {
    ...
    if (e.sigil) { award(run, 80, 6); this.hooks.notify('보호막 격파', 1.1, 2); return; }
    if (e.boss) {
      ...
      this.hooks.notify('보스 격파 · +2,000 · 하트 회복 · 오버드라이브', 5, 5);
  ...
  reflect(shot, run, perfect = false) {
    ...
    if (perfect) { gainFocus(run, 10); this.hooks.parried?.(); this.hooks.notify('완벽한 패리 · 송부 · +모래시계', 1.5, 4); this.hooks.sound.impact('storm'); }
  }
  ...
  startBoss(run) {
    ...
    this.hooks.notify(`${b.name} · ${b.shield ? '두 개의 보호막을 깨뜨려라' : '공격하거나 추월해라'}`, 4, 5); this.hooks.sound.roar(); this.shake = .5;
  }
  escapeBoss(run, outrun) {
    ...
    this.hooks.notify(outrun ? '보스 추월 · +350 · 하늘이 당신의 것' : '사냥이 멈춤 · 계속 비행', 3, 5);
    this.hooks.arena(false); this.hooks.bossUI(null);
  }
  updateBoss(dt, run) {
    const b = this.boss;
    if (!b) {
      if (run.distance > this.nextBoss - 180 && !this.warning) { this.warning = true; this.hooks.notify('무언가 거대한 것이 당신을 쫓고 있습니다…', 3, 4); this.hooks.sound.roar(); }
    ...
    if (bossPhase(b.hp, b.maxHp) > b.phase) {
      ...
      this.hooks.notify('분노 · 끝장을 봐라 아니면 지나쳐라', 2, 5); this.hooks.sound.roar();
    }
  ...
  update(dt, run, previousDistance, race = false) {
    ...
    for (let i = this.shots.length - 1; i >= 0; i--) {
      ...
      if (p.friendly) {
        ...
        if (run.roll) { award(run, 35, 4); this.hooks.notify('탄환 미끄러짐 · +스카이파이어', 1); } else this.hooks.hurt(); p.life = 0;
      }
